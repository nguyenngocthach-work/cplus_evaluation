...
<main class="flex-1 flex flex-col items-center py-8 px-4 md:px-10 lg:px-40">
  <div class="w-full max-w-[960px] flex flex-col gap-6">
    ...
    <div class="flex flex-wrap gap-2 px-4">
      ...
      <a class="text-[#617589] dark:text-gray-400 text-base font-medium leading-normal hover:text-primary transition-colors"
        href="<?php echo e(route('projects.screen')); ?>">Projects</a>
      <span class="text-[#617589] dark:text-gray-400 text-base font-medium leading-normal">/</span>
    ...
    <!-- Main Form Content -->
    <form id="project-form" method="POST" action="<?php echo e(route('projects.update', $project->project_id)); ?>" class="flex flex-col gap-6">
      ...
      <div
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
          <label class="flex flex-col gap-2 col-span-2 md:col-span-2">
            ...
            <input name="project_name" value="<?php echo e(old('project_name', $project->project_name)); ?>"
          ...
          <label class="flex flex-col gap-2 col-span-2 md:col-span-2">
            ...
            <textarea name="description"
              ...
              placeholder="Briefly describe the goals and scope of this project..."><?php echo e(old('description', $project->description)); ?></textarea>
          </label>
          <label class="flex flex-col gap-2">
            <span class="text-[#111418] dark:text-gray-200 text-base font-medium leading-normal">Start Date</span>
            <div class="relative">
              <input id="start-date" name="start_date" value="<?php echo e(old('start_date', $project->start_date ? \Carbon\Carbon::parse($project->start_date)->format('Y-m-d') : '')); ?>"
          ...
          <label class="flex flex-col gap-2">
            ...
            <div class="relative">
              <input id="end-date" name="end_date" value="<?php echo e(old('end_date', $project->end_date ? \Carbon\Carbon::parse($project->end_date)->format('Y-m-d') : '')); ?>"
                class="form-input w-full rounded-lg border border-[#dbe0e6] dark:border-gray-600 bg-white dark:bg-[#253240] text-[#111418] dark:text-white h-12 px-4 focus:ring-2 focus:ring-primary focus:border-primary transition-all"
                type="date" />
            </div>
          </label>
      ...
      <div
        ...
        <div class="p-6">
          ...
          <select name="status"
            ...
            <?php $__currentLoopData = $statusMap; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $st): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <option value="<?php echo e($key); ?>" <?php echo e($currentStatus == $key ? 'selected' : ''); ?>>
                <?php echo e($st['label']); ?>
        ...
        <div class="md:col-span-8">
          ...
          <div class="flex items-center gap-4">
            <div class="flex-1 bg-[#e5e7eb] dark:bg-[#4a5568] rounded-full h-2">
              <div
                class="<?php echo e($statusMap[$currentStatus]['bar']); ?> h-2 rounded-full transition-all duration-300"
                style="width: <?php echo e($statusMap[$currentStatus]['progress']); ?>%">
              </div>
            </div>

            <span class="text-sm font-bold text-[#111418] dark:text-white">
              <?php echo e($statusMap[$currentStatus]['progress']); ?>%
            </span>
          </div>

          <p class="mt-2 text-sm text-[#617589] dark:text-gray-400">
            <?php echo e($statusMap[$currentStatus]['label']); ?>
  ...
  <div id="criteriaModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
    ...
    <div class="bg-white dark:bg-[#1a2632] rounded-xl w-full max-w-lg shadow-lg">
      ...
      <div class="p-4 max-h-80 overflow-y-auto">
        <?php $__currentLoopData = $allCriteria; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div onclick="selectCriteria(<?php echo e(json_encode($item)); ?>)"
          class="flex justify-between items-center px-4 py-3 rounded-lg cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700">
          <div>
            <p class="font-medium"><?php echo e($item->criteria_name); ?></p>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    <?php if($project->client): ?>
        addClient(<?php echo json_encode([
            'id' => $project->client->id, 'client_name' => $project->client->client_name
        ], 512) ?>);
    <?php endif; ?>

    <?php $__currentLoopData = $project->industries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $industry): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      addLocation(<?php echo json_encode([
        'id' => $industry->id, 'industry_name' => $industry->industry_name
      ], 512) ?>);
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  });

  document.addEventListener('DOMContentLoaded', () => {
      <?php $__currentLoopData = $project->criteria; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <?php
              $criteriaData = [
                  'id' => $c->id,
          ...
          selectCriteria(<?php echo json_encode($criteriaData); ?>);
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  });
  </script>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/sit27847/domains/sitelocationadviser.com/public_html/evaluation/resources/views/project/project_update.blade.php ENDPATH**/ ?>